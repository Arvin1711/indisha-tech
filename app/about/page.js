...
const values = [
	{ icon: "🚀", title: "Product-Led", description: "We align business goals, user needs, and technology choices to build products that create measurable impact." },
	{ icon: "🔐", title: "Built Secure", description: "Security is integrated into every stage of delivery, from architecture and code reviews to deployment and monitoring." },
	{ icon: "🤝", title: "True Partnership", description: "We colindishaTech as an extension of your team with transparent communication, shared ownership, and clear priorities." },
	{ icon: "⚡", title: "Fast Execution", description: "Agile workflows, modern tooling, and senior engineering help us ship quality software quickly and consistently." },
];
...
export const metadata = {
	title: "Our Company — indishaTech",
	description: "Learn how indishaTech designs and builds scalable software products for startups and enterprises.",
};
...
const faqItems = [
	...
	},
	{
		...
		answer:
			"Yes. We frequently colindishaTech with in-house teams, providing dedicated developers, design support, architecture guidance, or end-to-end delivery depending on your needs.",
		expanded: false,
	},
	{
	...
	{
		question: "What technologies does indishaTech use?",
		answer:
		...
		<>
			...
			>
				...
				<section className="mx-auto max-w-7xl px-6 pt-10 text-center lg:px-10">
					...
					<h1 className="mx-auto mt-16 max-w-5xl text-[36px] font-normal leading-[0.99] tracking-[-0.07em] text-[#0d0f17] sm:text-[62px]">
						Building the future of{" "}
						<span className="font-[Georgia,serif] font-normal italic text-[#f08c54]">digital products</span>
					</h1>
					<p className="mx-auto mt-5 max-w-5xl text-[14px] sm:text-[16px] leading-[1.7] text-[#7a8192]">
						indishaTech was founded with one mission: help companies turn ideas into high-impact software. We
						combine product strategy, design thinking, and modern engineering to build secure, scalable digital
				...
				<section className="mx-auto mt-16 max-w-7xl px-6 lg:px-10">
					<div className="grid grid-cols-2 gap-3 rounded-2xl bg-white p-4 shadow-[0_20px_50px_-30px_rgba(15,23,42,0.12)] sm:grid-cols-3 sm:gap-4 sm:p-8">
						{[
							...
							>
								<p className="text-[24px] font-semibold tracking-tight text-[#0d0f17] sm:text-[32px]">{stat.value}</p>
								<p className="mt-1 text-[11px] text-[#8e96a7] sm:text-[12px]">{stat.label}</p>
							</div>
						))}
					</div>
				</section>
					...
					<div className="mt-3.5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
						{values.map((val) => (
							...
							>
								...
								<div>
									<h3 className="text-[14px] font-semibold text-[#1a1d28]">{val.title}</h3>
									<p className="mt-1 text-[13px] leading-[1.6] text-[#7a8192]">{val.description}</p>
								</div>
							</div>
						))}
					</div>
					...
					<div className="grid gap-8 lg:grid-cols-[1.15fr_0.55fr] lg:items-start">
						<div className="text-left">
							...
							<h3 className="mt-2 text-[28px] font-semibold tracking-[-0.02em] text-[#1c2029] sm:text-[34px]">
								Meet the People Behind indishaTech.
							</h3>
							<p className="mx-auto mt-2 max-w-3xl text-[12px] text-[#6f7686] sm:text-[13px]">
								From product vision to technical execution, our leadership team drives innovation and delivery excellence.
							</p>
						</div>

						<div className="ml-auto w-full rounded-lg max-w-67.5 bg-[#f7f7f8] p-5 sm:p-6">
							<p className="text-[36px] font-semibold leading-[0.9] tracking-[-0.04em] text-[#191d26]">25+</p>
							<p className="mt-1 text-[30px] font-semibold leading-[0.9] tracking-[-0.04em] text-[#191d26]">awards &amp;</p>
							<p className="text-[30px] font-semibold leading-[0.9] tracking-[-0.04em] text-[#191d26]">featured</p>
							<p className="mt-8 text-[12px] leading-[1.4] text-[#8a90a0]">
								Recognized for product innovation, engineering quality, and outstanding client outcomes.
							</p>
						</div>
					</div>
							...
							{leadershipMembers.map((member) => (
								...
								>
									...
									<div className="absolute inset-x-0 bottom-0 bg-linear-to-t from-black/72 via-black/32 to-transparent px-3 pb-3 pt-8 text-white">
										...
										<p className="mt-1 text-[11px] leading-[1.35] text-white/85">{member.role}</p>
									</div>
								</article>
							))}
						</div>
					...
					<div className="grid gap-4 lg:grid-cols-[1.2fr_0.9fr] lg:items-center">
						<div className="relative rounded-2xl bg-gray-50 p-5 sm:p-6">
							<div className="max-w-full space-y-5">
								{visions.map((item, index) => (
									<div key={index}>
										<p className="text-[20px] font-semibold text-[#1c2029] flex
											items-center gap-2">
						...
						<div className="px-2 text-center lg:px-6">
							...
							<p className="mx-auto mt-4 max-w-md text-[14px] leading-normal text-[#70788a]">
								Our vision and mission guide how we design, develop, and scale software products that solve real-world business challenges.
							</p>
						</div>
					</div>
								...
								{missions.map((item, index) => (
									<div key={index}>
										<p className="text-[20px] font-semibold text-[#1c2029] flex items-center gap-2">
											<MousePointerClick size={18} color="#980128" /> {item.title}
										</p>
										<p className="mt-1 text-[13px] leading-[1.45] text-[#1c2029] pl-6">
											{item.description}
										</p>
